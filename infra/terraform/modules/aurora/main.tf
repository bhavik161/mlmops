...
# ─────────────────────────────────────────────
# Aurora Cluster (Serverless v2 — auto-scales, scales to 0 for staging)
# ─────────────────────────────────────────────
resource "aws_rds_cluster" "main" {
  ...
  # PITR — required for RPO < 5 minutes
  backtrack_window = 0  # Not supported for PostgreSQL; use PITR
  ...
  serverlessv2_scaling_configuration {
    min_capacity = var.aurora_min_acu  # 0.5 for staging (scales to ~0), 2 for prod
    max_capacity = var.aurora_max_acu  # 16 for staging, 128 for prod
  }
